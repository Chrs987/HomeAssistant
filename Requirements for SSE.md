...
Charlie wants to be able to control his various IoT devices through Home Assistant in the living room using a spare tablet. Home Assistant offers a customizable dashboard built in called [Lovelace](https://www.home-assistant.io/lovelace/), which allows for Charlie to set up a simple dashboard for his tablet, to control his various IoT devices with ease, such as his Amazon Alexa, Thermostat, and lights. Once installed Charlie is prompted for his login information to his account, Charlie being the only person set up in his Home Assistant, provides the login information that was originally created during Home Assistants initialization, the Owner Account. Later that night Charlie invites his sister and her children over dinner, excited about his recent en devour, he shows his guests just how much control Home Assistant allows from his tablet.
#### Tablet Interface Use Case

Kaleb is one of Charlie's nephews who's more recently got into cybersecurity and hacking. Kaleb was extremely interested in Uncle Charlies integration with Home Assistant and while the adults where talking in the Kitchen, took Charlies Home Assistant integrated tablet to investigate. From the dashboard, Kaleb has access to the same configuration options as Charlie, more importantly access to all the in network IoT devices, as well as their configurations. Kaleb also has access to the raw YAML configuration of Charlies tablet UI, allowing him to accidentally wipe out all the customization Charlie has just set up. Kaleb also looks to add his own User to the Home Assistant System, which he can do, allowing him to access to all devices in the system from his own phone whenever he is connected to Charlies local network.

#### Prevention/Security Requirement

To prevent Kaleb from doing all of these changes to Charlies Home Assistant integration, Home Assistant gives administrator users the ability to create standard user accounts, allowing for read-only access to be set up for tablet systems. While this prevents Kalebs initial attack, arguments could be made that Charlie should have been notified to system changes to his Home Assistant, especially when his owner account credentials were use. Home Assistant does offer notifications from events happening in there [System Logs](https://www.home-assistant.io/integrations/system_log/), however those are only triggered with error or warning events NOT valid login or configuration changes. Eventually Home Assistant should provide better security practices similar to [CIS Benchmarks for AWS #1.1](https://docs.aws.amazon.com/securityhub/latest/userguide/securityhub-cis-controls.html), that recommend the 'owner' account is never used, so that you can set up proper IAM access to systems through created Users.
#### Diagram
