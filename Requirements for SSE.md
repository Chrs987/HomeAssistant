...
To prevent Kaleb from doing all of these changes to Charlies Home Assistant integration, Home Assistant offers different users to be created to the system, allowing for a read-only configuration to be set up for tablet systems; However guest access is still a Work In Progress from Home Assistant, only allowing a yes or no to whether or not a user created is a UI administrator. While this prevents Kalebs initial attack, arguements could be made that Charlie should have been notified to system changes to his Home Assistant, especially when his owner account credentials were use. Home Assistant does offer notifications from events happening in there [System Logs](https://www.home-assistant.io/integrations/system_log/), however those are only triggered with error or warning events NOT valid login or configuration changes. Eventually I wish for Home Assitant to provide security practicies similar to [CIS Benchamrks for AWS #1.1](https://docs.aws.amazon.com/securityhub/latest/userguide/securityhub-cis-controls.html), that recommend the 'owner' account is never used, so that you can set up proper IAM access to systems through created Users.
