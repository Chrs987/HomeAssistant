...
#### Tablet Interface Use Case

Charlie wants to be able to control his various IoT devices thorugh Home Assistant in the living room using a spare tablet. Home Assistant offers a customizable dashboards built in called [LoveLace](https://www.home-assistant.io/lovelace/), which allows for Charlie to set up a simple dashboard for his tablet, to control his various IoT devices with ease, such as his Amazon Alexa, Thermostat, and lights. Once installed Charlie is prompted for his login information to his account, Charlie being the only person set up in his Home Assistant, provides the login information that was orginially created during Home Assistants initaliazation, the Owner Account. Later that night Charlie invites his sister and her children over dinner, excited about his recent endevour, he shows his guests just how much control Home Assistant allows from his tablet.

#### Tablet Interface Use Case

Kaleb is one of Charlie's nephews whos more recently got into cyrbersecurity and hacking. Kaleb was extremely interested in Uncle Charlies integration with Home Assistant and while the adults where talking in the Kitchen, took Charlies Home Assistant integrated tablet to investigate. From the dashboard, Kaleb has access to the same configuration options as Charlie, more importantly access to all of the in network IoT devices, as well as their configurations. Kaleb also has access to the raw YAML configuration of Charlies tablet UI, allowing him to accidentally wipe out all of the customization Charlie has just set up. Kaleb also looks to add his own User to the Home Assistant System, which he can do, allowing him access to all devices in the system from his own phone whenever he is connected to Charlies local network.

#### Prevention/Security Requirement

To prevent Kaleb from doing all of these changes to Charlies Home Assistant integration, Home Assistant offers different users to be created to the system, allowing for a read-only configuration to be set up for tablet systems; However guest access is still a Work In Progress from Home Assistant, only allowing a yes or no to whether or not a user created is a UI administrator. In order to prevent other administrator actions, community created steps have been created to allow for a true guest user experience using Home Assistant, however it is important to note that this is NOT designed by Home Assistant. While this prevents Kalebs initial attack, arguements could be made that Charlie should have been notified to system changes to his Home Assistant, especially when his owner account credentials were use. Home Assistant does offer notifications from events happening in there [System Logs](https://www.home-assistant.io/integrations/system_log/), however those are only triggered with error or warning events NOT valid login or configuration changes. Eventually I wish for Home Assitant to provide security practicies similar to [CIS Benchamrks for AWS #1.1](https://docs.aws.amazon.com/securityhub/latest/userguide/securityhub-cis-controls.html), that recommend the 'owner' account is never used, so that you can set up proper IAM access to systems through created Users.
